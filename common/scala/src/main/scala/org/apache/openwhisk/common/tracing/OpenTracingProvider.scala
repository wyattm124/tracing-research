...
class OpenTracer(val tracer: Tracer, tracingConfig: TracingConfig, ticker: Ticker = SystemTicker) extends WhiskTracer {
  var spanMap = configureCache[String, List[Span]]()
  val contextMap = configureCache[String, SpanContext]()
  var spanListHold: List[Span] = Nil
  var set = 0
  override def getTracer = Some(tracer)
  ...
  override def setTraceContext(transactionId: TransactionId, context: Option[Map[String, String]]) = {
	...
	// NOTE : looking at the startSpan and finishSpan functions
	// it looks like changing the context is not enough
	// must also set the spanList to NULL so that the spans
	// made in this new context will actually be made off of This
	// new context and not off of the last span based in the old context
	if(set == 0) {
		println(" SET 0 !!!")
		spanListHold = spanMap.getOrElse(transactionId.meta.id, Nil)
		spanMap.put(transactionId.meta.id, Nil)
		set = 1
	} else {
		println(" SET 1 !!!")
		spanMap.put(transactionId.meta.id, spanListHold)
		set = 0
	}
  }
...
trait WhiskTracer {
  ...
  def getTracer(): Option[Tracer] = None
}
