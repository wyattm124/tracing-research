...
trait Container {
  ...
  def run(parameters: JsObject, environment: JsObject, timeout: FiniteDuration, maxConcurrent: Int)(
    implicit transid: TransactionId): Future[(Interval, ActivationResponse)] = {
		// MY EDITS : need to figure out if this is what runs our function for sure,
		// want to see the function parameters appear here for an invocation
		// then can move to start adding in our spans

		//switching contexts is not working, thus will create span explicityly

		// TEST 1 : what happens if I make new transaction Id out of
		// a given traceID???
		//implicit val newTransid = TransactionId(new TransactionMetadata("Test TransId", Instant.now()))
		val tracer = WhiskTracerProvider.tracer.getTracer
		var ctx_save: Map[String, String] = Map.empty[String, String]
		var temp_ctx: Map[String, String] = Map.empty[String, String]
		var option_ctx_save: Option[Map[String, String]] = WhiskTracerProvider.tracer.getTraceContext(transid)
		var trace_id: String = parameters.fields.get("_temp_trace_ID").map(_.convertTo[String]).getOrElse("")
		var parent_id: String = parameters.fields.get("_temp_parent_ID").map(_.convertTo[String]).getOrElse("")
		println(s"parameters : $parameters ***")
		println(s"env : $environment ***")
		println(s"trace ID : $trace_id ***")
		println(s"parent ID : $parent_id ***")
		if (trace_id == "" || parent_id == "") {
			println("NO Transaction ID detected (BUT NO CHANGE)***")
		} else {
			println("Transaction ID detected (BUT NO CHANGE)***")
			option_ctx_save match {
				case Some(a) => ctx_save = a
				case None =>
			}
			println(s"started ctx at : $ctx_save")
			if(!ctx_save.isEmpty) {
				// NOTE : leter it will be very easy to add prent spans
				// we just add it to the map
				temp_ctx = ctx_save + ("X-B3-TraceId" -> s"$trace_id", "X-B3-ParentSpanId" -> s"$parent_id")
				println(s"switched ctx to : $temp_ctx")
				// Note that it will not be enough to simply switch the context
				// as is implied. It may be that we need to also set the span map
				// for the transid to NIL so we wont be set to the child of a span
				// in the wrong context. A quick fix is in the Opentracing provider
				// file, and is commented
				WhiskTracerProvider.tracer.setTraceContext(transid, Some(temp_ctx))
			}
		}

    val actionName = environment.fields.get("action_name").map(_.convertTo[String]).getOrElse("")
	val ctx = WhiskTracerProvider.tracer.getTraceContext(transid)
	println(s"@ started got : $ctx")
	println(s"@ started used : $temp_ctx")
	WhiskTracerProvider.tracer.setTraceContext(transid, Some(temp_ctx))
    val start =
      transid.started(
        this,
    ...
    callContainer("/run", body, timeout, maxConcurrent, retry = false)
      .andThen { // never fails
        case Success(r: RunResult) =>
		  val ctx = WhiskTracerProvider.tracer.getTraceContext(transid)
		  println(s"@ finished got : $ctx") // this is what appears at server
		  println(s"@ finished used : $temp_ctx")
		  WhiskTracerProvider.tracer.setTraceContext(transid, Some(temp_ctx))
          transid.finished(
            this,
            start.copy(start = r.interval.start),
      ...
      .map { result =>
		...
		// See what Context map ENDS at
		WhiskTracerProvider.tracer.setTraceContext(transid, Some(ctx_save))
		println(s"ended ctx at : $ctx_save")
